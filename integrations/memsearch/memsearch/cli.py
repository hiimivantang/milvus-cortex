...
def expand(
    ...
    """
    from .store import CortexStore, _escape_filter_value
    ...
    try:
        escaped_hash = _escape_filter_value(chunk_hash)
        chunks = store.query(filter_expr=f'chunk_hash == "{escaped_hash}"')
        if not chunks:
...
# ======================================================================
# Cortex-specific commands
# ======================================================================


@cli.command("graph-extract")
@click.option("--source", "-s", default=None, help="Only extract from this source file.")
@click.option("--user-id", default=None, help="User ID for partition key scoping.")
@_common_options
def graph_extract(
    source: str | None,
    user_id: str | None,
    ...
    ms = MemSearch(**kwargs)
    try:
        result = _run(ms.extract_graph(source=source))
        entities = result.get("entities", [])
        relationships = result.get("relationships", [])
        click.echo(f"Extracted {len(entities)} entities and {len(relationships)} relationships.")
        for e in entities:
            click.echo(f"  Entity: {e.name} ({e.entity_type})")
        for r in relationships:
            click.echo(f"  Relationship: {r.source_id} --{r.relation_type}--> {r.target_id}")
    finally:
        ms.close()
...
def consolidate_cmd(
    ...
    """Run memory consolidation to merge similar chunks."""
    from .core import MemSearch
    ...
    ms = MemSearch(**kwargs)
    try:
        merged = _run(ms.consolidate(user_id=user_id))
        click.echo(f"Consolidated into {len(merged)} merged memories.")
    finally:
        ms.close()
...
def graph_search_cmd(
    ...
    ms = MemSearch(**kwargs)
    try:
        result = ms.graph_search(query, top_k=top_k)
        click.echo(json.dumps(result, indent=2, default=str))
    finally:
        ms.close()


@cli.command("cortex-stats")
@_common_options
def cortex_stats_cmd(
    provider: str | None,
    model: str | None,
    batch_size: int | None,
    base_url: str | None,
    ...
    """Show cortex runtime statistics and health."""
    from .core import MemSearch
